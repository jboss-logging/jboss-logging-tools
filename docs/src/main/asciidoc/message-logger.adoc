...
public interface AppLogger extends BasicLogger {
    ...
    @LogMessage(level = Logger.Level.WARN)
    @Throttled(period = 30, unit = TimeUnit.SECONDS) <5>
...
<5> Throttles the log message to at most once every 30 seconds. Supports count-based throttling (`count`), time-based throttling (`period` and `unit`), or both combined.